...
echo "Retrieving resource id for [$MANAGED_IDENTITY_NAME] managed identity..."
ID=$(az identity show \
  --name $MANAGED_IDENTITY_NAME \
  --resource-group $AKS_RESOURCE_GROUP_NAME \
  --query id \
...
if [[ -n $ID ]]; then
  echo "[$ID] resource id  for the [$MANAGED_IDENTITY_NAME] managed identity successfully retrieved"
else
  echo "Failed to retrieve resource id for the [$MANAGED_IDENTITY_NAME] managed identity"
  exit
fi
